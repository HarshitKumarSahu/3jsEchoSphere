...
class SoundReactor {
    ...
    setAudioSource(newUrl, isFile = false) {
...
class MainThreeScene {
    ...
    async init() {
        ...
        // New: File upload event listener
        ...
        document.getElementById('audio-file-input').addEventListener('change', (e) => {
          if (e.target.files.length > 0) {
            ...
            soundReactor.setAudioSource(fileURL, true);
            document.getElementById('audio-file-label').textContent = file.name;
            soundSrc.url = ''; // Clear Tweakpane dropdown to indicate custom file
            pane.refresh();
            const playPauseButton = document.getElementById('playPauseButton');
...
pane.addBinding(soundSrc, 'url', {
    ...
    document.getElementById('audio-file-label').textContent = 'No file selected';
    const playPauseButton = document.getElementById('playPauseButton');
    playPauseButton.disabled = false;
});
