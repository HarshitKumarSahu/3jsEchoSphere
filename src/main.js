...
class ParticleSystem {
  ...
  init(scene) {
    ...
    this.particlesGeom.setAttribute('position', new THREE.Float32BufferAttribute(this.particlesPos, 3));
    this.particleMaterial = new THREE.PointsMaterial({
      color: 0xffffff, // Bright purple to match UI
      size: 0.025,
      emissive: 0xffffff, // Add emissive property for bloom
...
class SpherePillerClass {
  ...
  init(scene) {
    ...
    this.gMatCap = new THREE.MeshMatcapMaterial({ 
      matcap: gTex,
      emissive: 0xEEABF6, // Add emissive property for bloom
...
class MainThreeScene {
  ...
  updatePostProcessing() {
    ...
    switch (this.postProcessingOptions.effect) {
      case 'none': // New case to handle no post-processing
        break;
      case 'bloom':
  ...
  update() {
    // Audio-reactive adjustments for each effect
    if (soundReactor && soundReactor.PARAMS) {
      ...
      if (this.postProcessingOptions.effect === 'bloom') {
        ...
        this.glitchPass.goWild = amplitude > 0.5; // Trigger wild glitch on high amplitude
        this.glitchPass.uniforms.byp.value = amplitude < 0.2 ? 1 : 0; // Bypass glitch when amplitude is low
